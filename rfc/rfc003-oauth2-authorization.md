...
Other terminology is taken from the Nuts Start Architecture.

## 3. OAuth flow
The mechanism of retrieving an access token using a JWT is based on the  JSON Web Token (JWT) Profile for OAuth 2.0 Client Authentication and Authorization Grants [RFC7523].
...
- **x5c**: contains both the signing certificate and the vendor CA. The first certificate MUST be the signing certificate. [RFC7515]
...
- **aud**: As per RFC7523, the aud MUST be the token endpoint. This can be taken from the Nuts registry. This is very important to prevent relay attacks.
- **usi**: User identity signature. The signed login contract according to [NATR]. Base64 encoded. Optional
...
The signed JWT MUST be sent to the registered authorization server oauth endpoint using the HTTP POST method. The `urn:ietf:params:oauth:grant-type:jwt-bearer` grant_type MUST be used. This profile is described in RFC7523. The JWT MUST be present in the assertion field. Below is a full HTTP example.
...
The usi field in the JWT contains the signed login contract. If present it MUST validate according to [NATR].
...
Errors are returned as described in https://tools.ietf.org/html/rfc6749#section-5.2:
...
### 6.2. Error codes
Different protocols return different types of error messages. The format will most likely also differ. This means that error messages have to be specified per use-case. If an error message supports a text-based error code, then it should support the illegal_access_token code. If a client receives this error code then it MUST NOT reuse the access token.

[NSA] “Nuts Start Architecture” (https://docs.google.com/document/d/1Mdq60H5ssFZp-T-9uyZUVXhiUU7Jg4orBQ61Y-qWGRs/edit?usp=sharing)
[NATR] “Nuts authentication token RFC” (https://docs.google.com/document/d/1OU9cEF6BdYgi-hONQXGIGF-t7gYyGiuAxy652M-iIg0/edit#)
[CSR] “Certificate Structure RFC”
[COIVR] “Care Organisation Identity Verification RFC”
[IAPP] https://irma.app
[IEXP] https://privacybydesign.foundation/irma-explanation/
[ATTR] https://privacybydesign.foundation/attribute-index/en/
