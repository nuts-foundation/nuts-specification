...
The **aud** field MUST match the URL of the registered oauth endpoint. This prevents the use of the JWT at any other endpoint. The concrete endpoint URL is used for this. \([RFC7523](https://tools.ietf.org/html/rfc7523#section-3)\). The `purposeOfUse` field identifies the service/Bolt which in turn refers to the correct endpoint URL. 
