...
An IRMA proof type acts as bridge between the IRMA ecosystem and Nuts ecosystem. Nuts uses DIDs, DID Documents and Verifiable Credentials as its base. Nuts node operators must trust specific DIDs in order for all features to work. IRMA publishes public keys of its issuers on Github. By trusting that specific repository and by running the IRMA server, a Nuts node operator can trust all Verifiable Credentials signed by IRMA. 
...
It MUST be a JSON object and MUST contain the scheme, the issuer, the credential and attribute name. The `email` and `domain` attributes would be contained as:
...
The `message` is used to connect the signatures and its attributes to a DID. This is the actual text the user will sign.
...
The verification result will indicate if the signature is valid.
The result will also contain the disclosed attributes as dot-delimited strings.
All attributes in the `credentialSubject` MUST be present in the result attributes.
The dot-delimited format of the verification result can be extended as follows:
...
Although IRMA supports revocation for credentials, this isn't usable for digital signatures.
The credential will be revoked at a certain timestamp, any signatures made in the past are still valid. 
...
* the IRMA signature in the `proofValue` MUST use the same attributes as the IRMA signature in the orignal VC. 
